function [Uhat, Ahat, info] = JacobiG_2P(A, delta_rel, maxsweep, U0, grad_eps, x_eps)
%JACOBI_G_2 Run the Jacobi-GP algorithm for nxnxL  
    ...
    for i=1:n
      for j=i+1:n
        ...
        if (abs(h0) >= delta * norm_Lambda_k) % Check the inequality
          iters = iters + 1; 
          [Psi_k,c_k,~,~] = find_Jacobi_P(W_k, i, j);
